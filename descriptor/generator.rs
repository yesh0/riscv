...
impl<'a> BitFieldDescriptor<'a>{
    ...
    fn getter(&self)->String{
        if self.lo==self.hi{
            ...
            return format!("{}::from(self.bits.get_bits({}..{}))", self.flag_type(), self.lo, self.hi+1)
        }else{
    ...
    fn setter(&self)->String{
        if self.lo==self.hi{
            ...
            return format!("self.bits.set_bits({}..{}, val as usize);", self.lo, self.hi+1)
        }else{
...
",
self.description, self.id, self.canonical_name(), self.id, self.canonical_name())
    }else{
        format!(
"
//! {}
